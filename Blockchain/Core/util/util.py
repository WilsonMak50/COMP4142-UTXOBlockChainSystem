...
from Blockchain.Core.EllepticCurve.EllepticCurve import BASE58_ALPHABET
...
def decode_base58(s):
    ...
    if hash_func(combined[:-4])[:4] != checksum:
        raise ValueError(f"bad Address {checksum} {hash_func(combined[:-4][:4])}")
...
def merkle_parent_level(hashes):
    ...
    for i in range(0, len(hashes), 2):
        parent = hash_func(hashes[i] + hashes[i + 1])
