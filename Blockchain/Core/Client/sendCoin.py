from Blockchain.Core.util.util import decode_base58
from Blockchain.Core.script import Script
from Blockchain.Core.transaction import Tx_In, Tx_Out, Tx
from Blockchain.Core.Database.database import AccountDB
from Blockchain.Core.EllepticCurve.EllepticCurve import PrivateKey
import time
import random 
class SendCoin:
            ...
            if self.Total < self.Amount:
                ...
                for index, txout in enumerate(TxObj.tx_outs):
                    if txout.script_pubkey.cmds[2] == self.fromPubKeyHash:
                        ...
                        TxIns.append(Tx_In(prev_tx, index))
            else:
                break
    ...
    def prepareTxOut(self):
        ...
        TxOuts.append(Tx_Out(self.Amount, to_scriptPubkey))

        """ Calculate Fee """
        self.fee = self.COIN
        self.changeAmount = self.Total - self.Amount - self.fee
        TxOuts.append(Tx_Out(self.changeAmount, self.From_address_script_pubkey))
        return TxOuts
    ...
    def prepareTransaction(self):
        ...
        if self.isBalanceEnough:
            ...
            self.TxObj = Tx(self.TxIns, self.TxOuts, 0)
